"""Render the dashboard window briefly and save a screenshot.

Used only by `npm run screenshot`-style smoke checks — not shipped
with the .exe.
"""
...
def main(out_path: Path, state: str = "BUSY") -> None:
    ...
    # Open the dashboard on the main thread; from a background thread
    # arrange for a screenshot then destroy it.
    def grab_and_quit() -> None:
        time.sleep(2.0)  # let CTk render once
        try:
